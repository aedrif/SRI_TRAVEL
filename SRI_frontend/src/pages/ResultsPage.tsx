...
const ResultsPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchResults = async () => {
            setIsLoading(true);
            try {
                const documents = await api.searchDocuments(query,10,"bm25",dest);
            ...
            <div className="flex-grow flex flex-col items-center justify-start py-24 px-4 relative">
                <div className="w-full max-w-7xl z-10 bg-white dark:bg-neutral-950 rounded-lg mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    {isLoading ? (
                        ...
                        >
                            <h2 className="text-2xl font-bold mb-6">Search Results for "{query}" {dest.length>0 ? "in "+dest.toUpperCase() : ''}</h2>
